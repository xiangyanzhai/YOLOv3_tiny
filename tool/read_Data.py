...
def handle_im(im, bboxes):
    ...
    im.set_shape(tf.TensorShape([1, None, None, 3]))
    bboxes = tf.concat([bboxes[..., :4] * scale, bboxes[..., 4:]], axis=-1)

    im = tf.pad(im, [[0, 0], [0, config.read_img_size - nh], [0, config.read_img_size - nw], [0, 0]],
                constant_values=127)
...
if __name__ == "__main__":
    ...
    files = [r'D:\PycharmProjects\Demo36\tf_cascade_rcnn\data_preprocess\voc_07.tf']
    # files = [r'D:\PycharmProjects\Demo36\SSD_mobilenet_v1\train_7\data_process\run.tf']
    # files = [r'D:\PycharmProjects\Demo36\SSD_mobilenet_v1\basketball_demo3\data_process_2\basketball_3.tf']
    # files = [r'D:\PycharmProjects\Demo36\SSD_mobilenet_v1\basketball_demo3\data_process_2\basketball_3.tf']
    files = [r'D:\PycharmProjects\Demo36\SSD_mobilenet_v1\basketball_demo3\data_process_2\basketball_3.tf']
    config = Config(files, jitter_ratio=[0.3], crop_iou=0.45)
